...
- **悬浮导出按钮**：开启后，在知乎页面右下角显示快捷导出悬浮球，点击即可快速导出当前内容。
- **最大下载回答数**：针对「问题页面」导出所有回答时，限制最大下载数量（默认 20 条，最大支持 50 条），避免请求过多导致卡顿。
...
1. 打开任意知乎页面：
   ...
   - 热榜页 (`zhihu.com/hot`)
2. 点击浏览器右上角的 **Zhihu to Markdown** 图标，或使用页面悬浮球（如果已开启）
3. 在弹出的窗口中确认信息后，点击「导出 Markdown」按钮

### 页面类型说明
- **专栏/回答**：直接导出当前文章或回答。
- **问题页面**：自动滚动加载多个回答后导出。此时会遵循设置中的「最大下载回答数」限制。
